...
public class HmacSigner {
    ...
    public static String generateSecret() throws HmacException {
        try {
            return Base64.encodeBase64String(generateToken().getBytes("UTF-8")).trim();
    ...
    public static Boolean isJwtExpired(String jwtString) throws ParseException {
        ...
        if(jwt.getJWTClaimsSet() != null && jwt.getJWTClaimsSet().getExpirationTime() != null) {
            OffsetDateTime expirationDate = OffsetDateTime.ofInstant(jwt.getJWTClaimsSet().getExpirationTime().toInstant(), ZoneId.systemDefault());
            return expirationDate.isBefore(OffsetDateTime.now(ZoneId.systemDefault()));
        } else {
            return false;
        }
    }
